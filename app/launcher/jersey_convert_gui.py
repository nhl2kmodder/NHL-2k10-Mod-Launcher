...
# Apply-scope groups: checkbox key -> (label, the sheets it covers). A colour sheet carries its
# stitched normal with it, so unticking "Body" also holds back base_normal.
APPLY_GROUPS = (
    ("base",    "Body",    ("base", "base_normal")),
    ("stamps",  "Stamps",  ("stamps", "normal")),
    ("letters", "Letters", ("letters", "letters_normal")),
...
SHOW_HELMET_SKIN_TAB = False    # the flat helmet-skin view; the Model tab paints the same skin
...
# "#" is the slot's ENTRY in the uniform's own table (DRAM 0x550 + N*16) -- the number the
# game's stamp pass indexes it by, and the order it lives in on disk.
...
class JerseyConvertTab:
    def __init__(self, app, root):
        ...
        # Sheets the user imported by hand, kept by name so they OUTLIVE a conversion — Convert
        # rebuilds the whole working set, and without this an imported PNG was silently replaced
        # by the conversion's own sheet, which Apply then wrote to the game files.
        self.imported: dict[str, Image.Image] = {}
        ...
        # The kit's own slot table as the game files hold it NOW (stamp_shader.kit_table), and
        # the uniform's ROS stamp flags. The boxes above are seeded FROM the table on load, so
        # what the editor shows is what the game samples; Apply writes the table back when it
        # differs. None until a kit is loaded / when the asset cannot be read.
        self.table_stock: dict | None = None
        self._kit_flags: dict | None = None
        self._stock_empty: set = set()   # slots the kit's table holds empty
        self.v_advance = StringVar(value="")
        self.v_model_sleeve_l = BooleanVar(value=True)
        self.v_model_laces = BooleanVar(value=False)
        self.v_model_sleeve_r = BooleanVar(value=True)
    ...
    def _build_controls(self, parent):
        ...
        # -- sources ----------------------------------------------------------------------
        g = ttk.LabelFrame(p_src, text="EA NHL textures", padding=8)
        g.pack(fill=X)
        ...
        g2 = ttk.LabelFrame(p_src, text="Uniform to edit", padding=8)
        ...
        # Which parts of the kit the conversion touches at all. Everything by default;
        # unticking a part keeps the kit's OWN sheet — in the preview, on the model, and in
        # Apply — so "just the stamps" or "just the helmet decal" can land without the rest
        # of the kit changing. The engine still runs in full (its glyphs and lifted logos
        # feed the ticked sheets); only the unticked results are discarded for stock.
        g4 = ttk.LabelFrame(p_src, text="Convert parts", padding=8)
        g4.pack(fill=X, pady=(10, 0))
        self.v_apply = {}
        for key, label, _sheets in APPLY_GROUPS:
            self.v_apply[key] = BooleanVar(value=True)
            ttk.Checkbutton(g4, text=label, variable=self.v_apply[key]).pack(
                side=LEFT, padx=(0, 12))
        ...
        ttk.Label(p_slot, foreground="#888", wraplength=390, justify=LEFT,
                  text="Drag a box on the Stamps preview to move it, or grab a corner to resize. "
                       ...
                       "These boxes are the GAME'S OWN windows: each uniform carries a 27-entry "
                       "table of sheet rectangles (# is the entry) and the stamp shader samples "
                       "exactly that rectangle, masked to it. They are read from the kit when it "
                       "loads, the model follows every edit live, and Apply writes the table back "
                       "into the uniform. Where a stamp lands ON THE BODY is still the mesh — to "
                       "move a crest on the jersey, move it inside its own slot art.\n\n"
                       "Digits: the box is the digit's INK rectangle and the advance is the "
                       "width of one digit's quad, so a wider advance spaces a two-digit number "
                       "out. A digit drawn bigger than its box is clipped by it — grow the box "
                       "too.\n\n"
                       "SIZE is how big the mark is drawn inside its cell, in percent. Click any "
                       "number to type a new one; click ON to turn a slot off (its entry is "
                       "written as empty).").pack(
            fill=X, pady=(0, 8))
        ab = ttk.Frame(p_slot); ab.pack(fill=X, pady=(0, 6))
        ttk.Label(ab, text="Digit advance (px)").pack(side=LEFT)
        sb = ttk.Spinbox(ab, from_=8, to=512, increment=1, width=5, textvariable=self.v_advance,
                         command=self._advance_changed)
        sb.pack(side=LEFT, padx=(6, 10))
        sb.bind("<Return>", lambda e: self._advance_changed())
        sb.bind("<FocusOut>", lambda e: self._advance_changed())
        self.v_table_note = StringVar(value="")
        ttk.Label(ab, textvariable=self.v_table_note, foreground="#888").pack(side=LEFT)
        ...
        for c, wd in zip(SLOT_COLS, (26, 30, 44, 44, 44, 44, 50)):
    ...
    def _build_normals_tab(self, nb):
        ...
        A 2K10 normal map is stitching: a raised bead either side of every panel join, a groove
        pulled down just inside the overlaid panel, and the cloth's weave in between. All of that
        is keyed to the STOCK stripe layout, so the moment a kit is recoloured or re-striped its
        relief is wrong — seams run through the middle of a solid panel and the new panel edges
        are glassy.

        The relief is STAMPED FROM MEASURED REFERENCE ART, not invented from parameters (see
        normal_templates.py). That is why this page has so few numbers on it now, and why they are
        all multipliers of 1: 1 means "exactly like the reference".
        ...
        ttk.Label(p, foreground="#888", wraplength=390, justify=LEFT,
                  text="Stitching is generated FROM the colour art: panel edges become sewn seams, "
                       "crests and numbers get an embroidered edge, panels get a weave. Stitch "
                       "after the colours are settled — the relief follows whatever the sheets "
                       "look like at the time.\n\n"
                       "The seam, weave and patch profiles are measured off reference art, so the "
                       "numbers below are multipliers: 1 is exactly the reference.").pack(
            fill=X, pady=(0, 8))
        ...
        g3 = ttk.LabelFrame(p, text="Start from", padding=8)
        g3.pack(fill=X, pady=(10, 0))
        # This replaced the old "Heal the stock stitching first" checkbox. Healing could only ever
        # remove relief it could FIND — and most of a base sheet's stitching sits in plain cloth,
        # where neither a colour edge nor a colour change selects it, so the old kit's piping kept
        # surviving into the new one. The clean template has none of it to begin with.
        self.v_ns_base = StringVar(value="template")
        ttk.Radiobutton(g3, text="A clean garment (recommended)", value="template",
                        variable=self.v_ns_base).pack(anchor=W)
        ttk.Label(g3, foreground="#888", wraplength=360, justify=LEFT,
                  text="The body sheet starts from the stock 2K10 garment — its panel seams, "
                       "cuffs, hem and collar, with no striping on it. Crests and numbers start "
                       "flat. Nothing of the old kit's stitching can survive.").pack(
            anchor=W, padx=(20, 0), pady=(0, 6))
        ttk.Radiobutton(g3, text="This kit's own normal, healed", value="stock",
                        variable=self.v_ns_base).pack(anchor=W)
        ttk.Label(g3, foreground="#888", wraplength=360, justify=LEFT,
                  text="Blurs the old seams out where the art changed and keeps the rest. Use it "
                       "when a kit has relief worth keeping and you only want to ADD to it.").pack(
            anchor=W, padx=(20, 0))

        g2 = ttk.LabelFrame(p, text="Stitch style", padding=8)
        g2.pack(fill=X, pady=(10, 0))
        # Typed numbers, not sliders: two of them are measurements the art dictates, and the rest
        # are multipliers of a measured reference, so the useful moves are small and exact.
        rows = (("color_thr", "panel threshold", "colour distance that counts as a new panel"),
                ...
                ("seam_scale", "seam width ×", "how wide a stripe's sewn edge runs. 1 = the "
                                               "reference art"),
                ("seam_h", "seam depth ×", "how proud the stripe seam sits. 1 = the reference art"),
                ("weave_h", "weave depth ×", "the cloth weave inside a stripe. Keep it subtle"),
                ...
                ("heal_blur", "heal radius px", "only used when starting from this kit's own "
                                                "normal"))
        self.v_ns = {}
        ...
        ttk.Button(b, text="Defaults", command=self.reset_normals).pack(side=LEFT, padx=6)
        ttk.Button(b, text="Revert to stock", command=self.revert_normals).pack(side=LEFT)
    ...
    def _ns_base_mode(self) -> str:
        return self.v_ns_base.get() if hasattr(self, "v_ns_base") else "template"
    ...
    def stitch_normals(self):
        ...
        stock_mode = self._ns_base_mode() == "stock"
        # Starting from the template needs only the NEW colour sheet: the floor comes from the
        # shipped template, so a kit that ships no letters_normal still gets one cut for it.
        have = [n for n in want
                if self.res.sheets.get(n) is not None
                and (not stock_mode or (self.stock.get(n) is not None
                                        and self.stock.get(NS.normal_key(n)) is not None))]
        if not have:
            self.v_ns_status.set("nothing to stitch — those sheets have no colour art"
                                 + (" / stock normal pair" if stock_mode else ""))
            return
        if self._stitch_sheets(have):
            self.model_refresh()

    def _stitch_sheets(self, have) -> bool:
        """Stitch these sheets' normals from the CURRENT colour art into the working set.
        Shared by the Stitch button and by sheet_import's auto-restitch. Returns True on success;
        does NOT re-render the model (callers do, so import renders once, not twice)."""
        ...
        try:
            ...
            out = NS.stitch_kit(self.stock, self.res.sheets, self._ns_params(),
                                ...
                                overlays=getattr(self.res, "font_normals", None))
    ...
    def _build_preview(self, parent):
        ...
        for s in SHEETS:
            ...
            # Round-trip a single sheet through an outside editor: export what the preview
            # shows, adjust, import it back — the model preview then wears the edit, and
            # Apply writes it. Packed BOTTOM first so the canvas takes the rest.
            bar = ttk.Frame(f)
            bar.pack(side=BOTTOM, fill=X, padx=4, pady=3)
            ttk.Button(bar, text="Export PNG…",
                       command=lambda n=s: self.sheet_export(n)).pack(side=LEFT)
            ttk.Button(bar, text="Import PNG…",
                       command=lambda n=s: self.sheet_import(n)).pack(side=LEFT, padx=6)
    ...
    # ── per-sheet export / import ────────────────────────────────────────────────────────
    def sheet_export(self, name):
        """Save this sheet exactly as the preview shows it, for editing outside the launcher."""
        img = (self.res.sheets.get(name) if self.res else None) or self.stock.get(name)
        if img is None:
            messagebox.showwarning("Jersey Editor", "Pick a kit first — nothing is loaded.")
            return
        stem = f"{self.v_team.get()}_{self.v_kit.get()}_{name}".lower() or name
        p = filedialog.asksaveasfilename(title=f"Export {SHEET_LABEL[name]} sheet",
                                         defaultextension=".png", initialfile=f"{stem}.png",
                                         filetypes=[("PNG image", "*.png")])
        if not p:
            return
        img.save(p)
        self.v_status.set(f"{SHEET_LABEL[name]} sheet exported to {p}")

    def sheet_import(self, name):
        """Bring an edited sheet back. It becomes THE sheet — shown here, painted on the model,
        written by Apply — resized to the archive's dimensions if needed, because the texture
        record's size is fixed. An import also re-ticks the part under 'Convert parts': handing
        a file over is the clearest possible statement that this part should land."""
        if self.res is None:
            messagebox.showwarning("Jersey Editor", "Pick a kit first — nothing is loaded.")
            return
        p = filedialog.askopenfilename(title=f"Import {SHEET_LABEL[name]} sheet",
                                       filetypes=[("Images", "*.png *.dds *.tga"),
                                                  ("All files", "*.*")])
        if not p:
            return
        try:
            img = J.load_image(p).convert("RGBA")
        except Exception as e:
            messagebox.showerror("Jersey Editor", f"Could not read {p}:\n{e}")
            return
        ref = self.res.sheets.get(name) or self.stock.get(name)
        note = ""
        if ref is not None and img.size != ref.size:
            note = f" (resized {img.size[0]}×{img.size[1]} → {ref.size[0]}×{ref.size[1]})"
            img = img.resize(ref.size, Image.LANCZOS)
        self.res.sheets[name] = img
        self.imported[name] = img        # survives a later Convert (reinstated in _converted)
        self.touched.add(name)
        for key, _label, sheets in APPLY_GROUPS:
            if name in sheets:
                self.v_apply[key].set(True)
        self.views[name].set_image(img)
        # A normal that was stitched for the OLD art is now wrong — re-cut it for this art
        # immediately, so the model and a later Apply track the import without another press.
        restitched = ""
        if name in NS.SHEET_ORDER and NS.normal_key(name) in self._stitched:
            restitched = " — re-stitched its normal" if self._stitch_sheets([name]) else ""
        self.model_refresh()
        self.v_status.set(f"{SHEET_LABEL[name]} sheet imported from {Path(p).name}{note}{restitched}")
    ...
    def _build_model_tab(self):
        ...
        cb.pack(side=LEFT, padx=(4, 12))
        cb.bind("<<ComboboxSelected>>", lambda e: self.model_refresh())
        # The two sleeve numbers are ROS flags too (+0x18 bits 20/19); seeded from the kit's
        # record by _sync_front_number, written back by Apply like the chest number.
        ttk.Checkbutton(bar, text="L sleeve #", variable=self.v_model_sleeve_l,
                        command=self.model_refresh).pack(side=LEFT, padx=(0, 6))
        ttk.Checkbutton(bar, text="R sleeve #", variable=self.v_model_sleeve_r,
                        command=self.model_refresh).pack(side=LEFT, padx=(0, 6))
        # The collar strings are a collar MESH, not art: ROS +0x18 bits 24..26 pick which of the
        # skater's five collars the kit wears and 1 is the laced one (Islanders); off goes back
        # to the plain V (Devils). Seeded by _sync_front_number, written by Apply.
        ttk.Checkbutton(bar, text="Collar strings", variable=self.v_model_laces,
        ...
        ttk.Label(f, foreground="#888", wraplength=560, justify=LEFT, padding=(6, 2, 6, 6),
                  text=("Drag to turn, wheel to zoom. " + (
                        "The figure (skater, gloves, helmet, head) and its decal quads are read "
                        "straight from the game's global.iff. "
                        if JP.mesh_source() == "game" else
                        "No game folder yet, so the figure is the shipped capture bake. ") +
                       "Every stamp is drawn from the uniform's OWN "
                       "data: the slot table in the uniform asset picks the sheet window for the "
                       "crest, letters, digits and patches (edit the boxes and the advance on the "
                       "Slots tab and this follows), and the roster's uniform flags pick the front "
                       "number placement, the sleeve numbers and which patch each site wears. "
                       "Front # and the sleeve toggles here edit those flags; Apply writes them. "
                       f"Still approximate or not drawn: {miss}.")).pack(
        fill=X)
    ...
    def _build_helmet_tab(self):
        ...
        """
        self.view_helm = None
        # Hidden by request (2026-08-18): the same skin is painted onto the 3D helmet on the
        # Model tab, and the flat unwrapped view wasn't earning its tab. Everything below still
        # works — flip the flag to bring it back.
        if not SHOW_HELMET_SKIN_TAB:
            return
    ...
    def _sync_front_number(self):
        ...
        try:
            from . import uniform_colors as ucol
            found = self._target_row()
            self._kit_flags = None
            if found:
                self._kit_flags = ucol.stamp_flags(*found)
                self.v_model_front.set(self._kit_flags["front"])
                self.v_model_sleeve_l.set(self._kit_flags["sleeve_left"])
                self.v_model_sleeve_r.set(self._kit_flags["sleeve_right"])
                self.v_model_laces.set(self._kit_flags.get("collar") == ucol.COLLAR_LACED)
        except Exception:
            pass

    def _laces_change(self):
        """The collar-strings edit waiting to be written: `(ros_path, row, on)` or None."""
        try:
            from . import uniform_colors as ucol
            found = self._target_row()
            if not found:
                return None
            ros, row = found
            want = bool(self.v_model_laces.get())
            # Style 1 (laced) or 0 (no-tab) and nothing else: the two CUSTOM collars live in
            # those two records, so a kit still on 2..4 (ANA's tab, EDM's) is moved onto 0.
            if ucol.collar_style(ros, row) == (ucol.COLLAR_LACED if want else 0):
                return None
            return str(ros.path), row, want
        except Exception:
            return None

    @staticmethod
    def _write_laces(change, log):
        path, row, on = change
        from . import ros_file as RF, uniform_colors as ucol
        ros = RF.RosFile(path)
        style = ucol.COLLAR_LACED if on else 0
        if ucol.collar_style(ros, row) == style:
            return
        ucol.set_collar_style(ros, row, style)
        ros.save()
        log("[jersey] collar strings -> %s (Roster.ROS uniform row %d, collar style %d)"
            % ("on" if on else "off", row, ucol.collar_style(ros, row)))

    def _sleeve_change(self):
        """The sleeve-number edit waiting to be written: `(ros_path, row, left, right)` or None.
        Same contract as `_front_number_change`."""
        try:
            from . import uniform_colors as ucol
            found = self._target_row()
            if not found:
                return None
            ros, row = found
            want = (bool(self.v_model_sleeve_l.get()), bool(self.v_model_sleeve_r.get()))
            if ucol.sleeve_numbers(ros, row) == want:
                return None
            return str(ros.path), row, want[0], want[1]
        except Exception:
            return None

    @staticmethod
    def _write_sleeves(change, log):
        path, row, left, right = change
        from . import ros_file as RF, uniform_colors as ucol
        ros = RF.RosFile(path)
        if ucol.sleeve_numbers(ros, row) == (left, right):
            return
        ucol.set_sleeve_numbers(ros, row, left, right)
        ros.save()
        log("[jersey] sleeve numbers -> L %s / R %s (Roster.ROS uniform row %d)"
            % ("on" if left else "off", "on" if right else "off", row))

    # ── the kit's slot table ─────────────────────────────────────────────────────────────
    def _seed_from_table(self, names=None):
        """Seed the slot boxes (and the advance) from the kit's own table.

        An entry the game holds EMPTY (w or h == 0) keeps the profile's box so there is something
        to drag, but starts OFF -- the two agree on what the game draws (nothing), and turning
        it on is what puts the box into the table.
        """
        t = self.table_stock
        slots = self.profile["stamps"]["slots"]
        if names is None:
            self._stock_empty = set()
        for n in (names if names is not None else self.slot_names):
            if n not in self.rects:
                continue
            ent = SS.DRAM_ENTRY.get(n)
            r = list(t["rects"][ent]) if (t and ent is not None) else None
            if r and r[2] > 0 and r[3] > 0:
                self.rects[n] = r
                self.enabled[n] = True
            else:
                self.rects[n] = list(slots[n]["rect"])
                self.enabled[n] = t is None
                if t is not None:
                    self._stock_empty.add(n)
        if t and (names is None):
            self.v_advance.set(str(t["advance"]))

    def _advance_px(self) -> int | None:
        try:
            v = int(float(self.v_advance.get()))
        except (ValueError, TypeError):
            return None
        return v if 8 <= v <= 512 else None

    def _advance_changed(self):
        # The advance only spaces the digit quads -- no art moves -- so just the model redraws.
        if self._advance_px() is not None:
            self.model_refresh()

    def _live_table(self) -> dict | None:
        """The table as the editor currently has it, in the game's entry order."""
        t = self.table_stock
        if t is None:
            return None
        rects = [list(r) for r in t["rects"]]
        for n, ent in SS.DRAM_ENTRY.items():
            if n in self.rects:
                rects[ent] = [max(0, int(v)) for v in self.rects[n]] \
                    if self.enabled.get(n, True) else [0, 0, 0, 0]
        adv = self._advance_px()
        return {"advance": adv if adv is not None else t["advance"],
                "row_h": t.get("row_h", 128), "rects": rects}

    def _table_change(self) -> dict | None:
        """The live table if it differs from what the uniform holds, else None."""
        live = self._live_table()
        if live is None or self.table_stock is None:
            return None
        same = (live["advance"] == self.table_stock["advance"]
                and all(list(a) == list(b) for a, b in zip(live["rects"],
                                                          self.table_stock["rects"])))
        return None if same else live

    def _live_flags(self) -> dict:
        """The ROS stamp flags with the model bar's unsaved edits laid over them."""
        f = dict(self._kit_flags or {})
        f["sleeve_left"] = bool(self.v_model_sleeve_l.get())
        f["sleeve_right"] = bool(self.v_model_sleeve_r.get())
        f["front"] = self.v_model_front.get()
        # Ticked = the laced collar (style 1, record 17); unticked = the no-tab collar (style 0,
        # record 16). Those two records hold the CUSTOM collars (build_collar.py), so a kit's own
        # 2..4 is never kept -- same rule as the write.
        try:
            from . import uniform_colors as ucol
            laced = ucol.COLLAR_LACED
        except Exception:
            laced = 1
        f["collar"] = laced if self.v_model_laces.get() else 0
        return f

    def _front_number_change(self):
        """The chest-number edit waiting to be written: `(ros_path, row, mode)` or None.

        `_sync_front_number` re-seeds the combobox from the target kit's OWN uniform record every
        time the target changes, so a value that still matches the record means the user never
        touched it and there is nothing to write. Resolved here, on the UI thread, because it
        reads Tk vars -- the apply worker gets a plain tuple and re-opens the roster itself.
        """
        try:
            from . import uniform_colors as ucol
            found = self._target_row()
            if not found:
                return None
            ros, row = found
            mode = self.v_model_front.get()
            if mode not in ("none", "small", "big") or ucol.front_number(ros, row) == mode:
                return None
            return str(ros.path), row, mode
        except Exception:
            return None

    @staticmethod
    def _write_front_number(change, log):
        """Write the chest-number placement into this uniform's ROS record (+0x18).

        Nothing in the ART decides this -- the stamps sheet carries one digit set either way --
        so the placement is a flag on the roster row and flipping it is the whole job. See
        `uniform_colors.FRONTNUM_BIG` / `FRONTNUM_SMALL`.

        The roster is re-opened from disk rather than carrying the RosFile across from the UI
        thread: the Uniforms tab edits the same file with the same read -> mutate -> save cycle,
        so writing back a copy read minutes ago would revert whatever it saved in between.
        """
        path, row, mode = change
        from . import ros_file as RF, uniform_colors as ucol
        ros = RF.RosFile(path)
        if ucol.front_number(ros, row) == mode:          # already there -- leave the file alone
            return
        ucol.set_front_number(ros, row, mode)
        ros.save()
        log("[jersey] chest number -> %s (Roster.ROS uniform row %d)" % (mode, row))
    ...
    def _model_render(self, ss=2):
        ...
        try:
            if self._model_dirty or self._model_scene is None:
                ...
                pair = self.targets.get(self.v_target.get())
                live = self._live_table()
                metrics = SS.table_metrics(
                    live, self._live_flags(),
                    logo=SS.kit_logo_params(pair[1]) if pair else None) if live else (
                    SS.kit_metrics(pair[1]) if pair else None)
                self._model_scene = JP.build_scene(
                    ...
                    collar=self._live_flags().get("collar"))
    ...
    def refresh_targets(self):
        ...
        self.targets = {}
        self.team_kits = {}
        # Expansion teams (SEA/VGK) live in EXTRA_TEAM_ROWS, which the IFF tab's roster scan
        # splices into the catalog — but that tab may not have been opened yet, and until it has,
        # this list would silently show only the 30 shipped teams. Run the scan ourselves when
        # the rows aren't installed; it is idempotent and logs its own reasons for a no-op.
        if not archtex.EXTRA_TEAM_ROWS:
            try:
                self.app._iff_discover_extra_teams()
            except Exception as e:
                self.app._log_q.put(f"[jersey] expansion-team scan failed: {e}")
        try:
    ...
    def load_stock(self):
        ...
        SS.clear_cache(uni_iff)
        self.table_stock = SS.kit_table(uni_iff, live=True)
        self._seed_from_table()
        self.v_table_note.set("from the uniform's own table" if self.table_stock
                              else "no slot table read — boxes are the profile's")
        self._fill_tree()
        self._push_boxes()
        self._stitched = set()
        self.imported = {}               # a fresh kit load is an explicit reset — imports too
        self.edit_mode = True
        self.touched = set()
    ...
    def _row_values(self, name):
        ...
        if name in EXTRA_SIZE_SHEET:
            return ("", "", "—", "—", "—", "—", pct)
        ent = SS.DRAM_ENTRY.get(name)
        return ("" if ent is None else str(ent), "✔" if self.enabled[name] else "",
                *self.rects[name], pct)
    ...
    def _toggle_slot(self, _e=None):
        ...
        if self.enabled[n] and not (self.rects[n][2] > 0 and self.rects[n][3] > 0):
            self.rects[n] = list(self.profile["stamps"]["slots"][n]["rect"])
        self._update_row(n)
        self._push_boxes()
    ...
    def reset_slots(self, everything):
        ...
        # "Reset" means the kit as the game files hold it -- the stock table -- and only the
        # profile's boxes when no table could be read.
        self._seed_from_table([n for n in names if n in self.rects])
        for n in names:
            self.sizes[n] = 1.0
            self._size_dirty.add(EXTRA_SIZE_SHEET.get(n, "stamps"))
        if everything and self.table_stock:
            self.v_advance.set(str(self.table_stock["advance"]))
        self._fill_tree()
        self._push_boxes()
        self._do_size()
        self._rebuild_stamps()
    ...
    def _opts(self):
        ...
        return J.Options(layers=None, strip_logos=bool(self.v_strip.get()),
                         ...
                         # A slot the kit's own table already holds EMPTY has no stock art
                         # under its box; 'off' there is the table entry, not an erase.
                         enabled={n: v or n in self._stock_empty
                                  for n, v in self.enabled.items()},
                         rects=dict(self.rects), art=art,
    ...
    def _converted(self, res, err):
        ...
        # Unticked "Convert parts" keep the kit's own art. The engine built the sheet anyway,
        # but the result is swapped back for the stock one and unmarked, so what you see and
        # what Apply writes both honour the tick.
        for key, _label, sheets in APPLY_GROUPS:
            if self.v_apply[key].get():
                continue
            for s in sheets:
                if self.stock.get(s) is not None:
                    res.sheets[s] = self.stock[s].copy()
                elif s in res.sheets:
                    del res.sheets[s]
                self.touched.discard(s)
        ...
        for sheet in NS.SHEET_ORDER:
            ...
            if key not in res.sheets and self.stock.get(key) is not None:
                ...
                self.touched.discard(key)
        # Hand-imported sheets OUTLIVE the conversion: handing a file over is the clearest
        # possible statement of what that sheet should be. Without this, Convert silently
        # replaced an imported PNG with its own sheet — the model then showed, and Apply then
        # wrote, the conversion's art instead of the user's.
        for name, img in self.imported.items():
            res.sheets[name] = img.copy()
            self.touched.add(name)
            for key, _label, group in APPLY_GROUPS:
                if name in group:
                    self.v_apply[key].set(True)
        self._stitched = set()
        for s in SHEETS:
        ...
        if res.collar_colors:
            names = {"A": "back", "B": "side", "C": "V", "D": "inside", "E": "laces",
                     "F": "eyelets"}
            self.txt_warn.insert(END, ("\n" if res.warnings else "") + "collar zones: " +
                                 "  ".join(f"{z} {names.get(z, z)} #{r:02x}{g:02x}{b:02x}"
                                           for z, (r, g, b) in sorted(res.collar_colors.items())))
    ...
    def _refresh_font_normal(self, sheet, o):
        ...
        the previous glyph's embroidery behind. Skipped silently when the source folder shipped
        no font normal, which is the common case.
        """
        if self.res is None or not getattr(self.res, "font_normals", None):
            return
        kw = dict(enabled=o.enabled, rects=o.rects) if sheet == "stamps" else {}
        ov = J.build_font_normal(self.res.glyphs, self.profile, sheet, self.stock.get(sheet),
                                 scales=o.scales.get(sheet), **kw)
        if ov is None:
            # Dragging a slot bigger can push EA's cell past MAX_FONT_UPSCALE, and a stale overlay
            # would then keep winning over a stitch that is now the better of the two.
            self.res.font_normals.pop(sheet, None)
        else:
            self.res.font_normals[sheet] = ov
    ...
    def _do_rebuild_stamps(self):
        ...
        try:
            ...
            self._refresh_font_normal("stamps", o)
    ...
    def _rebuild_letters(self):
        ...
        try:
            self.res.sheets["letters"] = J.build_letters(
                ...
                canvas=canvas)
            self._refresh_font_normal("letters", o)
    ...
    def apply(self):
        ...
        # The chest number is a ROSTER flag, not art, so it can be the only thing the user
        # changed -- it must not be swallowed by the "no sheets touched" / "all sheets unticked"
        # early-outs below.
        front = self._front_number_change()
        sleeves = self._sleeve_change()
        laces = self._laces_change()
        table = self._table_change()
        extra = front or sleeves or laces or table
        if not names and not extra:
            messagebox.showinfo("Jersey Editor", "Nothing has been changed yet.")
            return
        # The "Apply writes" checkboxes gate the write per garment part; an unticked part's
        # sheets (and their normals) simply stay as the archive has them.
        allowed = {s for key, _label, sheets in APPLY_GROUPS
                   if self.v_apply[key].get() for s in sheets}
        skipped = [n for n in names if n not in allowed]
        names = [n for n in names if n in allowed]
        if not names and not extra:
            messagebox.showinfo("Jersey Editor",
                                "Every changed sheet is unticked under 'Convert parts' — "
                                "tick at least one part to apply.")
            return
        if skipped:
            self.app._log_q.put("[jersey] held back by 'Convert parts': " + ", ".join(skipped))
        ask = []
        if names:
            ask.append(f"Write {', '.join(names)} into {pair[0]} and {pair[1]}?")
            if "stamps" in names:
                ask.append("The stamps sheet is also mirrored to this jersey's front-end "
                           "team-select art.")
        if front:
            ask.append(f"Set this kit's chest number to “{front[2]}” in Roster.ROS.")
        if sleeves:
            ask.append("Set this kit's sleeve numbers to L %s / R %s in Roster.ROS."
                       % ("on" if sleeves[2] else "off", "on" if sleeves[3] else "off"))
        if laces:
            ask.append("Put the collar strings %s in Roster.ROS (collar style %s)."
                       % ("on" if laces[2] else "off", "1, laced" if laces[2] else "0, no tab"))
        if table:
            ch = [SS.ENTRY_NAME.get(i, str(i)) for i, (a, b) in
                  enumerate(zip(table["rects"], self.table_stock["rects"])) if list(a) != list(b)]
            if table["advance"] != self.table_stock["advance"]:
                ch.insert(0, f"advance {self.table_stock['advance']}→{table['advance']}")
            ask.append(f"Write the slot table into {pair[1]} (in place): " + ", ".join(ch) + ".")
        if not messagebox.askyesno("Apply jersey", "\n\n".join(ask)):
            return
        sheets = {k: self.res.sheets[k].copy() for k in names}
        self.app._run_in_thread(self._apply_worker, pair, sheets, root, front, sleeves, table,
                                laces, op_label="Applying converted jersey…")

    def _apply_worker(self, pair, sheets, root, front=None, sleeves=None, table=None,
                      laces=None):
        log = self.app._log_q.put
        base_iff, uni_iff = pair
        # The roster flags go first: they are the edits that can stand alone, and a texture
        # write raising further down must not cost them.
        if front:
            try:
                self._write_front_number(front, log)
            except Exception as e:
                log(f"[jersey] chest number NOT written: {e}")
        if sleeves:
            try:
                self._write_sleeves(sleeves, log)
            except Exception as e:
                log(f"[jersey] sleeve numbers NOT written: {e}")
        if laces:
            try:
                self._write_laces(laces, log)
            except Exception as e:
                log(f"[jersey] collar strings NOT written: {e}")
        try:
            self._apply_sheets(pair, sheets, root, log)
        finally:
            # The table goes LAST: a whole-asset apply resets the uniform to its shipped bytes
            # first (`_reset_if_whole`), which would throw away a table written before it.
            if table:
                try:
                    log("[jersey] " + SS.write_kit_table(uni_iff, table, root, log))
                    self.table_stock = table
                except Exception as e:
                    log(f"[jersey] slot table NOT written: {e}")

    def _apply_sheets(self, pair, sheets, root, log):
        base_iff, uni_iff = pair
        ...
        try:
            ...
            for iff, wants in plan:
                edits, ntex = self._edits_for(iff, wants, paths, log)
                if not edits:
                    continue
                log(f"[jersey] {iff}: {len(edits)} texture(s)")
                self._reset_if_whole(iff, edits, ntex, root, log)
            ...
            if "stamps" in paths:
                for twin in jerseys.twins(uni_iff):
                    edits, ntex = self._edits_for(twin, [(0, "stamps")], paths, log)
                    if not edits:
                        continue
                    log(f"[jersey] mirror → {twin}")
                    self._reset_if_whole(twin, edits, ntex, root, log)
    ...
    @staticmethod
    def _reset_if_whole(iff, edits, ntex, root, log):
        """Reset `iff` to its shipped bytes, but ONLY when this apply rewrites every texture in it.

        `ensure_clean` throws the WHOLE asset back to stock so an apply never stacks a grow or a
        format conversion on a previous edit. That is right for a full apply and destructive for a
        partial one: in edit mode `names` is just the sheets the user touched, so resetting first
        silently reverted every sheet this pass was NOT rewriting. Measured on a live install --
        14 kits had their stamps/helmet/letters sitting byte-identical to stock with only the
        re-stitched normals applied, which is exactly this. The jersey path always calls
        replace_many with prefer_lossless=False (same dim, same format, in place), so there is no
        grow to stack and skipping the reset is safe.
        """
        if len(edits) >= ntex:
            archtex.ensure_clean(iff, root, log)
            return
        log(f"  partial apply ({len(edits)} of {ntex} textures) — keeping what is already in "
            f"the archive; no reset to stock")

    @staticmethod
    def _edits_for(iff, wants, paths, log):
        """-> ([replace_many edits], texture count), skipping anything this asset doesn't have.

        The count is what lets the caller tell a whole-asset apply from a partial one; see
        `_reset_if_whole`."""
        try:
