...
**Premult mip-level endian bug — FIXED (2026-07-27, in-game confirm pending).** All five mip-chain builders' *premultiplied* branches (`_rebuild_with_mips` square tail + generic walk, `_grow_many`, `replace_scene_hires`, `replace_multitex_grow`) called `ED.encode_image` **bare**, skipping the `_dxt_endian` 8-in-16 swap that `_encode_tiled` applies to mip0. Result: every mip below mip0 of a premult texture (team logos) was stored unswapped, and the GPU fetch (endian=1) scrambled it — swapped BC3 alpha endpoints (interpolation-mode flip → speckle) and pair-swapped pixel rows → "imports look perfect full-size but boxy/noisy when drawn small / at Xenia res scale 1". Proven offline against `logo_pit`/`logo_nsh`: clean mips decode only WITH the swap, pre-fix modded mips only WITHOUT it; post-fix re-apply restores the clean profile (MSE ~30–260 vs mip0 downscale). Straight-alpha textures always used `_encode_tiled` per level and were never affected (why portraits/UI didn't show it). Fix: those branches now route through `_encode_tiled(..., premultiply=False, alpha_aware=False)`. Any premult texture applied before 2026-07-27 has wrong mips baked in the archives — **re-Apply-All required** (done for the current mod set). **Verified in-game 2026-07-27** — small-scale logo renders confirmed clean after the fix + re-apply.

**Apply-All edit-scan filter bug — FIXED (2026-07-27).** `folders_with_edits()` returns absolute `Path`s but `asset_iff()` returns folder strings relative to `Textures/Extracted` — the `in` check in `_apply_all_mods` never matched, so Apply All found 0 texture jobs ("Nothing to apply"). The call site now normalizes the folder list to relative posix strings against `Textures/Extracted` and `Textures/Modified` before comparing.
